...
import PoolTypeMatrixChart from '@site/src/components/Dashboard/PoolTypeMatrixChart';
import LoadingSpinner from '@site/src/components/common/LoadingSpinner';

export function PoolTypeMatrix() {
  const [selectedSegment, setSelectedSegment] = useState(null);

  const handleSegmentClick = (pool, type) => {
    setSelectedSegment({ pool, type });
  };
    ...
    <div style={{ padding: '24px 0' }}>
      ...
      {selectedSegment && (
        <div style={{
          ...
          <strong>Selected:</strong> {selectedSegment.pool} → {selectedSegment.type}
        </div>
      )}
    </div>
  );
