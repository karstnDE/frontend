...
<BrowserOnly fallback={<LoadingSpinner />}>
  {() => {
    ...
    function PoolTypeMatrix() {
      ...
      useEffect(() => {
        Promise.all([
          fetch('/analytics/data/top_transactions_pool.json').then(res => res.json()),
          fetch('/analytics/data/summary.json').then(res => res.json())
        ])
      ...
      const handleSegmentClick = (poolId, poolLabel) => {
        setSelectedSegment({ poolId, poolLabel });
      };
          ...
          {selectedSegment && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span><strong>Filtered by:</strong> {selectedSegment.poolLabel}</span>
                <button
