...
export function PoolRevenueDashboard() {
  ...
  return (
    <div style={{ padding: '24px 0' }}>
      {/* Revenue Breakdown Chart - Pool View */}
      <BreakdownChart
        summary={summary}
        groupMode="pool"
      ...
      {selectedPoolLabel && (
        <div style={{
          ...
          <span style={{ fontSize: '14px', color: 'var(--ifm-color-secondary)' }}>
            Filtered by: <strong>{selectedPoolLabel}</strong>
          </span>
          <button
            onClick={clearFilter}
            style={{
      ...
      <TopTransactionsTable
        ...
        groupMode="pool"
        selectedFilter={selectedPoolId}
        selectedFilterLabel={selectedPoolLabel}
        summary={summary}
      />
    </div>
  );
