...
This page documents the analytical approaches specific to DefiTuna. For general methodology, see [Data Pipeline & Methodology](../methodology).

## Overview

DefiTuna analytics currently cover three main areas:
...
Each area requires specialized approaches tailored to DefiTuna's unique characteristics. Each of those areas have in commong that they can be analysed from looking at transactions that touch the Treasury account.
...
- **Which pool and DEX** - Orca SOL-USDC, Fusion pools, etc.
...
USDC is actually a bad example because it gets swapped to SOL multiple times per day. But as the last swap might not exactly take place at 00:00 AM UTC, there will be always some token "hangover".

### Dual Verification

Two independent methods calculate total revenue:
1. **Simple count** - Sum all SOL inflows (i.e. revenue earned in SOL, and the SOL coming from SwapReward)
2. **Attributed sum** - Sum all attributed SOL from the ledger

These must match 100%. Any discrepancy indicates an attribution logic error requiring investigation.
...
Two APR metrics are calculated:
...
I track this behavior across different staker sizes: (AI TO CHECK AGAINST CURRENT COHORTS IN CODE)
...
A "user" is a unique wallet address that signs transactions interacting with DefiTuna's Treasury Account. I track signers rather than just recipients to focus on active participants.

### Activity Categories

**Stakers** - Wallets that stake, unstake, claim rewards, or compound. These represent committed users directly participating in the protocol's staking mechanism. AI TO CHECK - SHOULD BE WALLETS WITH ACTIVE STAKE

**Daily Active Users** - Wallets involved in revenue-generating activities like fee collection, position adjustments, liquidity operations, etc. Excludes pure token transfers. AI TO CHECK - SHOULD BE DAILY WALLETS
...
- **USDC, JUP** - Common revenue tokens (examples)
