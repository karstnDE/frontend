...
<BrowserOnly fallback={<LoadingSpinner />}>
  {() => {
    ...
    const { useApyData } = require('@site/src/hooks/useApyData');

    function ApySummaryStats() {
      const { data, loading } = useApyData();
...
<BrowserOnly fallback={<div>Loading example...</div>}>
  {() => {
    ...
    function ExampleCalculation() {
      const { data: apyData, loading } = useApyData();
      const [data, setData] = useState(null);

      useEffect(() => {
        if (apyData?.daily_apr) {
          // Get the last entry (most recent date with full 30-day window)
          const fullWindowData = apyData.daily_apr.filter(d => d.rolling_days === 30);
      ...
      return (
        <div>
          ...
          <ul>
            <li>30-day rolling revenue: ${formatCurrency(data.rolling_revenue_usd)} USD ({formatCurrency(data.rolling_revenue_sol)} SOL)</li>
...
{`Annualized Revenue = ($${formatCurrency(data.rolling_revenue_usd)} / ${data.rolling_days}) × 365 = $${formatCurrency(data.annualized_revenue_usd)} USD
Revenue per TUNA = $${formatCurrency(data.annualized_revenue_usd)} / 1,000,000,000 = $${formatPrice(data.revenue_per_tuna_usd)} USD

Reference APR = ($${formatPrice(data.revenue_per_tuna_usd)} / $${formatPrice(data.tuna_price_usd)}) × 100 = ${data.reference_apr_percent.toFixed(2)}%
Entry Price APR = ($${formatPrice(data.revenue_per_tuna_usd)} / $0.05) × 100 = ${data.your_apr_percent.toFixed(2)}%`}
          </pre>
