...
1. Enter a Solana wallet address in the search box below
2. Click "Analyze Wallet" or press Enter
3. View the wallet's complete staking history with:
   - **Stacked area chart**: Staked + unstaked TUNA balance over time
   - **Dashed line**: Realized rewards (claimed + compounded)
   - **Colored dots**: Individual operations (stake, unstake, compound, claim, withdraw)
   - **Interactive tooltips**: Click on dots to view transaction details and links to Solscan

**Pro Tip**: You can also share a direct link with a pre-filled wallet address by adding a URL parameter:
        ...
        <div style={{ padding: '0' }}>
          ...
          {data && data.found && !loading && (
            <>
              ...
              {(() => {
                const pageSize = 10;
                const totalPages = Math.ceil(data.operations.length / pageSize);
                const startIdx = currentPage * pageSize;
                const endIdx = startIdx + pageSize;
                const pageOperations = data.operations.slice(startIdx, endIdx);
                ...
                return (
                  <div className="wallet-transactions-table" style={{
                    ...
                    <h3 style={{ marginTop: 0, marginBottom: '16px' }}>All Transactions ({data.operations.length})</h3>
                    ...
                    {totalPages > 1 && (
                      <div style={{
                        ...
                        <div style={{ fontSize: '14px', color: 'var(--ifm-color-secondary)' }}>
                          Showing {startIdx + 1}-{Math.min(endIdx, data.operations.length)} of {data.operations.length}
                        </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  ℹ️ <strong>Note:</strong> Realized rewards are shown in SOL (claimed and compounded amounts). Unclaimed rewards are not shown (planned for Phase 2).
                </div>
              </div>
            </>
          )}
